...
export const buttonVariants = cva("flex items-center justify-center gap-2", {
  variants: {
    ...
    variant: {
      primary:
        "border border-border-primary bg-button-primary text-text-white hover:bg-button-hover transition ease-in duration-100",
      secondary: "bg-button-secondary text-text-primary",
      ghost:
        "border border-border-primary bg-transparent hover:bg-bg-platinum transition ease-in duration-100",
  ...
  (
    ...
    return (
      <Component
        ref={ref}
        className={cn(
          buttonVariants({ size, radius, weight, variant }),
          className
        )}
        {...props}
      >
