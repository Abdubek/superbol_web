...
import { SearchByFilter } from "@/features/SearchByFilter";
...
export const ScoutParticipantsTable = ({
  cities,
  participants,
  totalCount,
}: Props) => {
    ...
    <>
      ...
      <div className="relative border border-border-lightgray rounded-md p-4 flex flex-col gap-2">
        <div>
          ...
          <SearchByFilter startTransition={startTransition} />
        </div>
              ...
              {participants?.map((item, index) => (
                <tr key={index} className="border-b border-border-lightgray">
                  ...
                  <td className="px-6 py-4 flex items-center gap-4 h-[80px]">
                    ...
                    <div>
                      <div className="line-clamp-1 mb-1">{item.full_name}</div>
